(function () {
  ...
  const WORLD_COLORS = {
    "sandar-qalasy": { solid: "var(--blue)", tint: "rgba(29,78,116,.08)" },
    "bolshekter-zertkhanasy": { solid: "var(--green)", tint: "rgba(31,109,70,.08)" },
    "pishinder-sheberkhanasy": { solid: "var(--sand-dark)", tint: "rgba(147,112,31,.10)" },
    "logika-ekspeditsiyasy": { solid: "var(--terracotta)", tint: "rgba(162,74,47,.09)" },
  };

  function worldColor(worldName) {
    const world = state.curriculum.worlds.find((w) => w.name === worldName);
    return (world && WORLD_COLORS[world.id]) || WORLD_COLORS["bolshekter-zertkhanasy"];
  }
  ...
  function renderHeroWorlds() {
    ...
    wrap.innerHTML = state.curriculum.worlds.map((w) => `
      <div class="world-chip">
        <span class="world-dot" style="background:${(WORLD_COLORS[w.id] || WORLD_COLORS["bolshekter-zertkhanasy"]).solid}"></span>
  ...
  function renderKtzhTable(filterText = "", filterWorld = "") {
    ...
    tbody.innerHTML = rows.map((m) => {
      const color = worldColor(m.world);
      return `
      <tr>
        <td>${m.num}</td>
        ...
        <td>${m.topic}</td>
        <td>${m.outcome}</td>
        <td><span class="world-tag" style="background:${color.tint};color:${color.solid};">${m.world}</span></td>
        <td>${m.theory}</td>
        <td>${m.practice}</td>
        <td>${m.total}</td>
        <td>
  ...
  function renderKmzCards(filterText = "", filterWorld = "") {
    ...
    grid.innerHTML = items.map((m) => {
      const color = worldColor(m.world);
      return `
      <article class="mission-card" id="kmz-${m.code}" style="--card-accent:${color.solid}">
        <div class="mission-top">
          <span class="mc-badge" style="background:${color.solid}">${m.code}</span>
          <span class="mc-world" style="background:${color.tint};color:${color.solid};">${m.world}</span>
        </div>
        <h4>${m.topic}</h4>
        <p class="mc-outcome">${m.outcome}</p>
        <div class="mc-meta"><span>Ұзақтығы: 45 минут</span><span>Ойын форматы: ${m.gameFormat}</span></div>
